...
def get_dataloader(dataset, is_train, args, image_encoder=None):
    """
    兼容三种 dataset 输入：
    1) 你 repo 的封装对象：有 train_loader / test_loader
    2) torch Dataset（如 torchvision.datasets.ImageFolder）
    3) 已经是 DataLoader（直接返回）
    同时保留 feature_dataset 模式
    """
    ...
    # 情况1：封装对象有 train_loader/test_loader
    if hasattr(dataset, "train_loader") and hasattr(dataset, "test_loader"):
        return dataset.train_loader if is_train else dataset.test_loader

    # 情况2：dataset 是 torch Dataset（ImageFolder 等）——自己包一层 DataLoader
    return DataLoader(
        dataset,
        batch_size=args.batch_size,
        shuffle=is_train,
